...
OLD_PATH = "Users/amy/Desktop/new_soundhub/audio"
NEW_PATH = "workspaces/bird_new/samples/non-avian_ML/audio"
...
def create_train_sizes(df):
    ...
    while len(current_df) > rows_to_remove_per_iteration * 2:
        ...
        if (
            len(df_neg) < rows_to_remove_per_iteration
            or len(df_pos) < rows_to_remove_per_iteration
        ):
            print("Not enough rows to continue sampling.")
            break

        neg_to_remove = df_neg.sample(
            n=rows_to_remove_per_iteration, random_state=iteration
        )
        pos_to_remove = df_pos.sample(
            n=rows_to_remove_per_iteration, random_state=iteration
        )
...
def process_species_model(
    species, model_name, audio_dir, results_dir, batch_size, epoch, learning_rate
):
    """
    ...
    train_df["file"] = train_df["file"].apply(
        lambda file: file.replace(OLD_PATH, NEW_PATH)
    )
    test_df["file"] = test_df["file"].apply(
        lambda file: file.replace(OLD_PATH, NEW_PATH)
    )
    ...
    for train_size, i in dataframes_list.items():
        ...
        emb_train = model.embed(
            train, return_dfs=False, batch_size=batch_size, num_workers=0
        )
        emb_val = model.embed(
            test_df, return_dfs=False, batch_size=batch_size, num_workers=0
        )

        quick_fit(
            model.network, emb_train, train.values, emb_val, test_df.values, steps=1000
        )
    ...
    print("Finished!")
    results_df.to_csv(
        f"{results_dir}/{species}-{model_name}-{batch_size}-{epoch}-{learning_rate}.csv",
        index=False,
    )


def run_training(
    species_list, model_list, audio_dir, results_dir, batch_size, epoch, learning_rate
):
    """
...
SPECIES_LIST = [
    "human_vocal"
]  # , 'engine', 'woodhouses_toad','pacific_chorus_frog','bullfrog','field_cricket','coyote']

AUDIO_DIR = "/workspaces/bird_new/samples/non-avian_ML/audio"
RESULTS_DIR = "/workspaces/bird_new/samples/non-avian_ML/results"
